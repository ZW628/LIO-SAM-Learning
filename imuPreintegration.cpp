...
public:
    ...
    // imu数据队列
    //imuQueOpt用来给imuIntegratorOpt_提供数据来源，
    //不要的就弹出(从队头开始出发，比当前激光里程计数据早的imu通通积分，用一个扔一个)；
    std::deque<sensor_msgs::Imu> imuQueOpt;
    //imuQueImu用来给imuIntegratorImu_提供数据来源，
    //不要的就弹出(弹出当前激光里程计之前的imu数据,预积分用完一个弹一个)；
     ...
     * 施加从该时刻开始到当前时刻的imu预积分量，得到当前时刻的状态，也就是imu里程计
    ...
    */
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        // predict odometry
        // 用上一帧激光里程计时刻对应的状态、偏置，施加从该时刻开始到当前时刻的imu预计分量，
        //得到当前时刻的状态
        ...
        //odometryHandler函数中根据prevPose_ = lidarPose.compose(lidar2Imu)得到激光帧先验位姿(lidarPose)转换到imu系下,（相当于从真正的雷达系转到上面的中间系中）
        //作为初值构建了因子进行优化；
